...
def tag_special_ingredients(dish):
    ...
    ingredients = dish[1]
    in_special_ingredients = set(ingredients).intersection(SPECIAL_INGREDIENTS)
    return (dish[0], in_special_ingredients)
...
def compile_ingredients(dishes):
    ...
    merged_ingredients = set().union(*dishes)
...
def separate_appetizers(dishes, appetizers):
    ...
    final_list = list(dishes_set.difference(appetizers_set))
    return final_list
...
def singleton_ingredients(dishes, intersection):
    ...
    # Compute the union of all ingredients across all dishes
    all_ingredients = set().union(*dishes)
    # Return ingredients that are not in the intersection set
